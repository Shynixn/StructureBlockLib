...
public class CraftStructureBlockIT {
    ...
    /**
     * Given a structure block with properties
     * when saving and loading
     * all data should be correctly saved and retried.
     */
    @Test
    public void saveStructureLoadStructure_ChangedStructure_ShouldCorrectlySaveAndLoad() {
        // Arrange
        MockedProxyService proxyService = new MockedProxyService();
        NBTTagCompound input = new NBTTagCompound();
        input.setString("mirror", "LEFT_RIGHT");
        ...
        classUnderTest.setSaveName("Thisismysavename");
        classUnderTest.setBlockNameMetaData("Thisismetadata");
        classUnderTest.setStructureLocation(new Location(Mockito.mock(World.class), 30, 10, 40));
        classUnderTest.setSizeX(10);
        classUnderTest.setSizeY(20);
        classUnderTest.setSizeZ(5);
        classUnderTest.setIncludeEntities(true);
        classUnderTest.setBoundingBoxVisible(true);
        classUnderTest.setInvisibleBlocksEnabled(true);
        classUnderTest.setIntegrity(0.4F);
        ...
        classUnderTest
                .saveStructure()
                .saveToString()
                .onResult(e -> wrap.item = e);
        while (wrap.item == null) {
            while (!proxyService.concurrentList.isEmpty()) {
                proxyService.concurrentList.remove().run();
            }
        ...
        String actualSavedContent = wrap.item;
        classUnderTest
                .loadStructure()
                .loadFromString(actualSavedContent)
                .onResult(e -> wrap.item = null);
        while (wrap.item != null) {
            while (!proxyService.concurrentList.isEmpty()) {
                proxyService.concurrentList.remove().run();
            }
        ...
        // Assert
        Assertions.assertEquals("H4sIAAAAAAAAAONiYOBkYCnOrEplZmBgAGE44GTgSM0rySzJTC2G8tmScvKTsyE8Dga2xNKSjPwiBkZ7Zgb2stSi4sz8PKAEIwMAgo+FiFQAAAA=", actualSavedContent);
    }
